# 📜 GameOS Tool - Changelog

All notable changes to GameOS Tool are documented in this file.

---

## [1.0.3] - 2026-08-06
- **EN**: Ram Cleaner improved to eliminate in-game micro-stutters. Task Scheduler tasks now automatically delete and re-create upon update.
- **TR**: RAM Temizleyici oyun içi takılmaları önleyecek şekilde geliştirildi. Güncelleme sonrası Görev Zamanlayıcı görevlerinin otomatik silinip yenilenmesi sağlandı.

---

## [1.0.2] - 2026-08-06
- **EN**: "Ram Cleaner" has been improved to provide better experience while gaming.
- **TR**: "RAM Temizleyici" oyunlarda daha iyi performans alınabilmesi için geliştirildi.

---

## [1.0.1] - 2026-08-05
- **EN**: Performance improvements and bug fixes.
- **TR**: Performans iyileştirmeleri ve hata düzeltmeleri.

---

## [1.0.0] - 2026-08-01
- **EN**: Initial release of GameOS Tool.
- **TR**: GameOS Tool ilk sürüm yayınlandı.
